...
vis_dir <- file.path(here("vis"), paste0(identifier, "_k", k))
dir.create(vis_dir, recursive = TRUE)
imap(jsons, ~ serVis(
                json = .x,
                out.dir = file.path(vis_dir, .y),
                open.browser = FALSE)
)
...
edu_doc_list <- map2(ldas, topic_orders, ~ lda_to_docs(.x, .y))
edu_topic_list <- map2(ldas, topic_orders, ~ lda_to_topics(.x, .y))
...
top_terms <- map(edu_topic_list, ~ get_top_terms(.x, 15))
...
### secondary education sub-subset
sec_edu_topicnum <- list(
    `1978-90` = c(3, 7, 8, 9, 24),
    `1990-01` = c(14, 17, 18, 23, 3),
    `2001-14` = c(5, 8, 10, 12, 22, 23),
    `2014-20` = c(1, 3, 5, 6, 9, 13),
    `all`     = c(2, 7, 14, 15)
)
sec_edu_docs <- map2(edu_doc_list, sec_edu_topicnum,
                     ~filter(.x, topic %in% .y)) %>%
                map(. %>%
                    group_by(topic) %>%
                    filter(gamma > quantile(gamma, 0.90)
                  ))
sec_edu <- map2(edu,
                sec_edu_docs,
                ~get_edu_corpora(corpus = .x,
...
# there is only one speech in this period
# fucks the wordfish pipeline up
sec_edu <- map(sec_edu, ~filter(.x, !Period == "1977-1979"))

count_n <- function(corpus) {
  corpus <- corpus[-5] %>%
    bind_rows %>%
    get_political %>%
  parties <- corpus %>%
  ...
  periods <- corpus %>%
    ...
    group_by(timeseries) %>%
    summarize(n(), )
  return(list(parties = parties, blocs = blocs, periods = periods))
}

sec_obs <- count_n(sec_edu)

sec_obs %>%
  bind_rows %>%
  xtable %>%
  print.xtable(file = 'tmp/obs.tex',
...
edufiles <- list.files(path = here("data/parties_on_edu/"),
                       pattern = str_c(".*", "txt"))
edupolicy <- map(edufiles, ~ read_file(str_c(here("data/parties_on_edu/"), .x)))
  names(edupolicy) <- c("Dansk Folkeparti",
                        "Enhedslisten",
                        "Konservative Folkeparti",
                        "Radikale Venstre",
                        "Socialdemokratiet",
...
policy <- enframe(edupolicy) %>% unnest %>% rename(Parti = name, text = value)

policy_fish <- policy %>%
    unnest_tokens(lemma, text, token = "ngrams", n = n) %>%
    count(lemma, Parti, sort = TRUE) %>%
    bind_tf_idf(lemma, Parti, n) %>%
    cast_dtm(Parti, lemma, n) %>%
    as.wfm %>%
    wordfish
...
fish_latest <- edu[[4]] %>%
  ...
  get_political %>%
  filter(Parti %in% parties) %>%
  corpus_to_fish("Parti", n)

s1 <- summarize_fish(policy_fish) %>%
  filter(Gruppe %in% keep) %>%
  mutate(Gruppe = fct_relevel(s1$Gruppe, keep))

s2 <- summarize_fish(fish_latest) %>%
  filter(Gruppe %in% keep) %>%
  mutate(Gruppe = fct_relevel(s2$Gruppe, keep))

s3 <- bind_rows(s1, s2)

p <- ggplot(s2, aes(x = Gruppe, y = Estimate)) +
     geom_point(aes(color = "Seneste regeringsperiode") ) +
     geom_point(data = s1, aes(color = "Partiprogrammer")) +
...
ggsave(p,
       filename = "wordfish_compare.tex",
       path = here("fig/"),
       device = tikz,
       width = 5,
       height = 5)

sec_fish_partyXperiod <- wordfish_corpus(corpus = sec_edu, timeperiod = "timeseries", group = "Parti", n = n, filters = parties, filter_col = "Parti")
sec_fish_partyXgovt <- wordfish_corpus(corpus = sec_edu, timeperiod = "Period", group = "Parti", n = n, filters = parties, filter_col = "Parti")
sec_fish_periods <- wordfish_corpus(corpus = sec_edu, timeperiod = "timeseries", n = n, filters = blocs, filter_col = "Blok")
sec_fish_blocXgovt <- wordfish_corpus(corpus = sec_edu, timeperiod = "Period", group = "Blok", n = n, filters = blocs, filter_col = "Blok")
sec_fish_blocXperiod <- wordfish_corpus(corpus = sec_edu, timeperiod = "timeseries", group = "Blok", n = n, filters = blocs, filter_col = "Blok")

fish_partyXperiod <- wordfish_corpus(corpus = edu, timeperiod = "timeseries", group = "Parti", n = n, filters = parties, filter_col = "Parti")
fish_partyXgovt <- wordfish_corpus(corpus = edu, timeperiod = "Period", group = "Parti", n = n, filters = parties, filter_col = "Parti")
fish_periods <- wordfish_corpus(corpus = edu, timeperiod = "timeseries", n = n, filters = blocs, filter_col = "Blok")
fish_blocXgovt <- wordfish_corpus(corpus = edu, timeperiod = "Period", group = "Blok", n = n, filters = blocs, filter_col = "Blok")
fish_blocXperiod <- wordfish_corpus(corpus = edu, timeperiod = "timeseries", group = "Blok", n = n, filters = blocs, filter_col = "Blok")
...
# use this in wordfish_corpus when collating on "Blok"
# cast_dtm() hiccups otherwise
bloc_to_fish <- function(corpus, group, n) {
  cat(str_c("Collating tokens on group: ", group, "\n"))
  corpus %>%
    select(text, all_of(group)) %>%
    group_by(across({{ group }})) %>%
...
get_coef_terms <- function(coef_tibble, n = 10) {
  ...
  max_psi <- coef_tibble %>%
    ...
    slice_max(psi, n = 5, with_ties = FALSE) %>%
    select(period, token) %>%
    summarise(neutral1 = paste(token, collapse = ", "))
  min_psi <- coef_tibble %>%
    group_by(across(period)) %>%
    filter(psi > quantile(psi, 0.85)) %>%
    slice_min(psi, n = 5, with_ties = FALSE) %>%
    select(period, token) %>%
    summarise(neutral2 = paste(token, collapse = ", "))
  ...
  terms <- inner_join(min_beta, max_psi) %>%
    inner_join(min_psi) %>%
    inner_join(max_beta) %>%
    unite(neutral, c(neutral1, neutral2), sep = ", ")
}
...
coef_blocXperiod <- coef_fishlist(sec_fish_blocXperiod)
terms_blocXperiod <- get_coef_terms(coef_blocXperiod)
write_term_xtable(terms_blocXperiod, identifier)
...
plot_coef_fish <- function(fish, psi = TRUE) {
  ...
  words <- words %>%
    filter(!token %in% t)
  p <- p +
  geom_label_repel(data = slice_max(words, beta, n = 5, with_ties = FALSE),
                  aes(label = token)
                  ) +
  geom_label_repel(data = slice_min(words, beta, n = 5, with_ties = FALSE),
                  aes(label = token)
                  )  +
  geom_label_repel(data = slice_max(words, psi, n = 5, with_ties = FALSE),
                  aes(label = token)
                  ) +
  geom_label_repel(data = slice_min(filter(words, psi > quantile(psi, 0.85)), psi, n = 5, with_ties = FALSE),
                  aes(label = token)
                  )
}
...
p <- plot_coef_fish(sec_fish_periods)
save_coef_plot(p, identifier)
...
plot_fishlist_blocs <- function(fishlist_blocs) {
   p <-  periods_fishlist(fishlist_blocs) %>%
     ...
     geom_line(aes(color = Gruppe)) +
     theme(legend.position = "bottom") +
     scale_color_manual(values = c("blue", "yellow", "red", "gray"))
}

plot_fishlist_parties <- function(partyfish) {
   partyfish %>%
     plot_fishlist +
     geom_line(aes(linetype = Gruppe)) +
     theme(legend.position = "bottom") +
...
p <- plot_fishlist_blocs(sec_fish_blocXperiod)
write_wordfish_timeseries_plot(p, str_c(identifier, "_blocs_period"))

p <- plot_fishlist_blocs(sec_fish_blocXgovt)
write_wordfish_timeseries_plot(p, str_c(identifier, "_blocs_govt"), width= 8)
...
partyXperiod <- periods_fishlist(sec_fish_partyXperiod) %>%
  filter(Gruppe %in% keep)
p <- plot_fishlist_parties(partyXperiod)
write_wordfish_timeseries_plot(p, str_c(identifier, "_parties_period"), height = 4, width = 6)
...
read_ann <- function() {
  filenames <- list.files(
              path = here("data/"),
              pattern = str_c("speeches_annotated_\\d*.rds"
              )
  )
  cat("found annotations:\n")
  map(filenames, ~ cat(str_c(.x, "\n")))
  ann <- map(filenames, ~select(readRDS(here(str_c("data/", .x))), c(sentence, sentence_id, doc_id))) %>%
    bind_rows()
}
...
sec_sentences <- filter_ann(annotated, sec_edu)
...
edu_sent <- find_sentiments(sentences, sentence) %>% tally_sentiments %>% aggregate_sentiments
sec_sent <- find_sentiments(sec_sentences, sentence) %>% tally_sentiments %>% aggregate_sentiments
...
edu_doc_sent <- find_sentiments(edu_pol, text) %>%
  rename(doc_sent_mean = sent_mean) %>%
  rename(doc_sent_total = sent_total)
sec_doc_sent <- find_sentiments(sec_pol, text) 
  rename(doc_sent_mean = sent_mean) %>%
  rename(doc_sent_total = sent_total)

edu_sent_pol <- inner_join(edu_sent, edu_doc_sent) %>%
  mutate(timeseries = as.factor(timeseries)) %>%
  mutate(Period = as.factor(Period)) %>%
  mutate(Parti = as.factor(Parti))
sec_sent_pol <- inner_join(sec_sent, sec_doc_sent) %>%
  mutate(timeseries = as.factor(timeseries)) %>%
  mutate(Period = as.factor(Period)) %>%
  mutate(Parti = as.factor(Parti))


edu_bloc_sent <- filter(edu_sent_pol, Blok %in% blocs)
sec_bloc_sent <- filter(sec_sent_pol, Blok %in% blocs)

edu_party_sent <- filter(edu_sent_pol, Parti %in% parties)
sec_party_sent <- filter(sec_sent_pol, Parti %in% parties)

plot_sent_spread <- function(corpus) {
  corpus %>%
    # requires a munged data corpus pivoted to include these columns
    ggplot(aes(x = Parti, y = value, color = value > 0 )) +
    guides(color = "none") +
    # facet_grid(timeseries ~ .)  +
    geom_point(alpha = 0.9) +
    scale_color_manual(values = c("lightsalmon", "palegreen1")) +
    theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
    xlab(element_blank()) +
    ylab("Spredning")
}

plot_sent_meanminmax <- function(corpus, name) {
  p <- corpus %>%
    mutate(Parti = fct_relevel(Parti, keep)) %>%
    pivot_longer(c(sent_mean_min, sent_mean_max), names_to = "variable", values_to = "value") %>%
    plot_sent_spread
  ggsave(p,
           device = tikz,
           filename = str_c("sent_meanminmax_", name, ".tex"),
           path = here("fig/"),
           width = 5,
           height = 5
    )
...
p<-plot_sent_meanminmax(filter(edu_sent_pol, Parti %in% keep), "edu")
p<-plot_sent_meanminmax(filter(sec_sent_pol, Parti %in% keep), "sec_edu")

plot_sent_minmax <- function(corpus, name) {
  p <- corpus %>%
    mutate(Parti = fct_relevel(Parti, keep)) %>%
  ...
  ggsave(p,
           device = tikz,
           filename = str_c("sent_minmax_", name, ".tex"),
           path = here("fig/"),
           width = 5,
           height = 5
    )
...
p <-plot_sent_minmax (filter(edu_sent_pol, Parti %in% keep), "edu")
p <-plot_sent_minmax (filter(sec_sent_pol, Parti %in% keep), "sec_edu")


plot_doc_total <- function(corpus, name) {
  p <- corpus %>%
    mutate(Parti = fct_relevel(Parti, keep)) %>%
    ggplot(aes(x = Parti, y = doc_sent_total, color = doc_sent_mean > 0 )) +
    guides(color = "none") +
    # facet_grid(timeseries ~ .)  +
    geom_point(alpha = 0.9) +
    scale_color_manual(values = c("lightsalmon", "palegreen1") ) +
  ...
  ggsave(p,
           device = tikz,
           filename = str_c("doc_sent_total_", name, ".tex"),
           path = here("fig/"),
           width = 5,
           height = 5
    )
...
plot_doc_mean <- function(corpus, name) {
  p <- corpus %>%
    mutate(Parti = fct_relevel(Parti, keep)) %>%
    ggplot(aes(x = Parti, y = doc_sent_mean, color = doc_sent_mean > 0 )) +
    guides(color = "none") +
    # facet_grid(timeseries ~ .)  +
    geom_point(alpha = 0.9) +
    scale_color_manual(values = c("lightsalmon", "palegreen1") ) +
  ...
  ggsave(p,
           device = tikz,
           filename = str_c("doc_sent_mean_", name, ".tex"),
           path = here("fig/"),
           width = 5,
           height = 5
    )
...
p <-plot_doc_total (filter(edu_sent_pol, Parti %in% keep), "sec_edu")
p <-plot_doc_mean (filter(sec_sent_pol, Parti %in% keep), "sec_edu")

# FIXME: the following four functions are very nearly
#        identical. Bad DRY.

get_postot_sentences <- function(corpus, sentences) {
  c <- corpus %>%
    group_by(Parti) %>%
    slice_max(sent_total_max, n = 3) %>%
    select(Parti, doc_id, sent_total_max, doc_sent_total, doc_sent_mean, sentiments, text) %>%
    ungroup %>%
    unnest(cols = c(sentiments)) %>%
    filter(sent_total == sent_total_max)
  s <- sentences %>%
    filter(doc_id %in% c$doc_id) %>%
    select(sentence, sentence_id, doc_id)
  inner_join(c, s)
}

get_negtot_sentences <- function(corpus, sentences) {
  c <- corpus %>%
    group_by(Parti) %>%
    slice_min(sent_total_min, n = 3) %>%
    select(Parti, doc_id, sent_total_min, doc_sent_total, doc_sent_mean, sentiments, text) %>%
    ungroup %>%
    unnest(cols = c(sentiments)) %>%
    filter(sent_total == sent_total_min)
  s <- sentences %>%
    filter(doc_id %in% c$doc_id)
  inner_join(c, s)
}

get_posmean_sentences <- function(corpus, sentences) {
  c <- corpus %>%
    group_by(Parti) %>%
    slice_max(sent_mean_max, n = 3) %>%
    select(Parti, doc_id, sent_mean_max, doc_sent_total, doc_sent_mean, sentiments, text) %>%
    ungroup %>%
    unnest(cols = c(sentiments)) %>%
    filter(sent_mean == sent_mean_max)
  s <- sentences %>%
    filter(doc_id %in% c$doc_id)
  inner_join(c, s)
}

get_negmean_sentences <- function(corpus, sentences) {
  c <- corpus %>%
    group_by(Parti) %>%
    slice_min(sent_mean_min, n = 3) %>%
    select(Parti, doc_id, sent_mean_min, doc_sent_total, doc_sent_mean, sentiments, text) %>%
    ungroup %>%
    unnest(cols = c(sentiments)) %>%
    filter(sent_mean == sent_mean_min)
  s <- sentences %>%
    filter(doc_id %in% c$doc_id)
  inner_join(c, s)
}

totpos <- get_postot_sentences(filter(sec_sent_pol, Parti %in% keep), sec_sentences)
totneg <- get_negtot_sentences(filter(sec_sent_pol, Parti %in% keep), sec_sentences)
meanpos <- get_posmean_sentences(filter(sec_sent_pol, Parti %in% keep), sec_sentences)
meanneg <- get_negmean_sentences(filter(sec_sent_pol, Parti %in% keep), sec_sentences)

get_negpos_docs <- function(corpus) {
  posneg <- list(
    neg = ~slice_min(.x, n = 3),
    pos = ~slice_max(.x, n = 3)
  )
  neg = corpus %>%
    group_by(Parti) %>%
      slice_min(doc_sent_mean, n=3) %>%
    ungroup
  pos = corpus %>%
    group_by(Parti) %>%
      slice_max(doc_sent_mean, n=3) %>%
    ungroup
  negtot = corpus %>%
    group_by(Parti) %>%
      slice_min(doc_sent_total, n=3) %>%
    ungroup
  postot = corpus %>%
    group_by(Parti) %>%
      slice_max(doc_sent_total, n=3) %>%
    ungroup
  bind_rows(pos, neg, negtot, postot)
}

get_negpos_blocdocs <- function(corpus) {
  posneg <- list(
    neg = ~slice_min(.x, n = 3),
    pos = ~slice_max(.x, n = 3)
  )
  neg = corpus %>%
    group_by(timeseries) %>%
      slice_min(doc_sent_mean, n=5) %>%
    ungroup
  pos = corpus %>%
    group_by(timeseries) %>%
      slice_max(doc_sent_mean, n=5) %>%
    ungroup
  negtot = corpus %>%
    group_by(timeseries) %>%
      slice_min(doc_sent_total, n=3) %>%
...
d <- get_negpos_blocdocs(filter(edu_sent_pol, Blok %in% blocs)) %>%
  select(text, doc_sent_mean, Parti, timeseries, doc_id) %>%
  arrange(timeseries, doc_sent_mean)
xtable(d) %>% print.xtable(file = 'tmp/doc_bloc_sent.html',
                           type = 'html',
                           include_rownames =FALSE)

coll <- bind_rows(totpos, totneg, meanpos, meanneg) %>%
  select(-c(sent_mean_sign, sent_total_sign)) %>%
  xtable %>%
  print.xtable(file = 'tmp/sent.html',
               type = "html",
               include.rownames = FALSE)


sentences %>%
  filter(doc_id == 780743) %>%
  select(sentence)
  filter(sentence_id == 1)

plot_sent_time <- function(corpus, group, period) {
  corpus %>%
  # pivot_longer(c(sent_mean, sent_total), names_to = "variable", values_to = "value") %>%
  ggplot(aes(y = doc_sent_mean, x = Date)) +
    # facet_grid(group) +
    ...
    # scale_x_continuous(breaks = corpus$Period) +
    ylab("Gennemsnitlig holdningsværdi, på talebasis") +
    xlab("Tid")
}

# WIP: the above crashes luatex bc too many points
#      attempting the bgimg hack
plot_sentiment_time <- function(corpus, group, period) {
  p <- ggplot(corpus, aes(y = doc_sent_mean, x = Date)) +
    geom_point(aes_string(color = period))
  img_plot <- p +
    theme_void() +
    theme(strip.text = element_blank())
...
p <- plot_sentiment_time(filter(edu_sent_pol, Parti %in% keep), "Parti", "timeseries")
write_sent_time(p, "edu")

p <- plot_sent_time(filter(sec_sent_pol, Parti %in% keep), "Parti", "timeseries")
write_sent_time(p, "sec_edu")

p <- plot_sent_time(filter(edu_sent_pol, Blok %in% blocs), "Blok", "timeseries")
write_sent_time(p, "sec_edu")

write_sent_time <- function(p, name) {
ggsave(p,
       device = pdf,
       filename = str_c("sent_spread_time_", name, ".pdf"),
       path = here("fig/"),
       width = 8,
       height = 3
       # standAlone = FALSE
...
plot_sent_score <- function(corpus, name) {
  corpus %>%
    group_by(Parti) %>%
...
plot_sent_party_count <- function(corpus, name) {
counted <- corpus %>%
  group_by(Parti) %>%
    summarize(across(c(sent_mean_sumpos, sent_mean_sumneg), ~sum(.x > 0), .names = "count_{.col}")) %>%
  mutate(ratio = signif(count_sent_mean_sumpos / count_sent_mean_sumneg), digits = 1) %>%
  mutate(count_sent_mean_sumneg = count_sent_mean_sumneg * -1) %>%
  pivot_longer(c(count_sent_mean_sumpos, count_sent_mean_sumneg), names_to = "pol", values_to = "count")
  glimpse(counted)
  ratios = filter(counted, pol == "count_sent_mean_sumpos")
  p <- ggplot(counted, aes(x = reorder(Parti, count), y = count)) +
       geom_bar(stat="identity", aes(fill = count > 0)) +
       geom_label(data = filter(counted, pol == "count_sent_mean_sumpos"), aes(label = ratio), y = 50) +
       coord_flip() +
       guides(fill = FALSE) +
       ylab("Antal positive og negative taler") +
       xlab(element_blank())
  ggsave(p,
         filename = str_c("count_sent_", name, ".tex"),
         path = here("fig/"),
         device = tikz,
         width = 7,
         height = 5
...
plot_sent_party_count(filter(sec_sent_pol, Parti %in% keep), "sec_edu_parties")
